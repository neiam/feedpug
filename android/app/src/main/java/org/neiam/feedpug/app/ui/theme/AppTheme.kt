...
// Ported from the shared neiam OKLCh palettes (same set the FeedPug/dms/diogramos
// web frontends use), so themes look identical across the apps.
...
data class AppTheme(
    ...
    fun toColorScheme() = if (isLight) {
        lightColorScheme(
            ...
            // Pin Material's surfaceContainer tones to the theme card colour so
            // cards/menus/dialogs actually look themed instead of neutral grey.
...
/** Plain Material 3 light defaults — straight from M3's baseline light theme. */
...
val ALL_THEMES: List<AppTheme> = listOf(
    // FeedPug's signature theme and default — navy + cyan with a yellow accent.
    // Values mirror the "blueprint" daisyUI theme in the web app's app.css.
    oklchTheme("Blueprint",  "blueprint",  0.28f, 0.12f, 252f,  0.24f, 0.11f, 252f,  0.91f, 0.05f, 235f,  0.68f, 0.18f, 248f,  0.55f, 0.05f, 235f),
    AppTheme(
        name = "Light", key = "light",
        bg = MaterialLight.BG, cardBg = MaterialLight.CARD, content = MaterialLight.CONTENT,
        primary = MaterialLight.PRIMARY, dim = MaterialLight.DIM, accent = MaterialLight.ACCENT,
        isLight = true,
    ),
