defmodule FeedPug.Accounts.Invite do
  @moduledoc """
  A single-use registration invite. When `FeedPug.Accounts.registration_open?/0`
  returns false, `/users/register` only accepts visitors carrying a valid invite
  token in the URL (`/users/register?invite=...`).
  """
  use FeedPug.Schema
  import Ecto.Changeset

  alias FeedPug.Accounts.User
  ...
  @doc """
  Stamps the invite as consumed by the given user. Idempotent: a second call
  returns the same record without re-stamping.
  """
