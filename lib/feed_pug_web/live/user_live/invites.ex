defmodule FeedPugWeb.UserLive.Invites do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> stream(:invites, Accounts.list_invites(user))}
  end
    ...
    case Accounts.list_invites(user) |> Enum.find(&(to_string(&1.id) == id)) do
      ...
      invite ->
        case Accounts.revoke_invite(user, invite) do
          {:ok, revoked} -> {:noreply, stream_insert(socket, :invites, with_redeemer(revoked))}
          _ -> {:noreply, put_flash(socket, :error, "Could not revoke invite.")}
        end
    end
  end

  defp with_redeemer(invite), do: FeedPug.Repo.preload(invite, :consumed_by)
          ...
          >
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 text-xs">
                <span :if={invite.consumed_at && invite.consumed_by_id} class="badge badge-ghost">
                  used
                </span>
                <span
                  :if={invite.consumed_at && is_nil(invite.consumed_by_id)}
                  class="badge badge-error"
                >
                  revoked
                </span>
                <span :if={is_nil(invite.consumed_at)} class="badge badge-success">active</span>
              </div>
              <code class="block mt-1 text-xs font-mono truncate">
                {url(~p"/users/register?invite=#{invite.token}")}
              </code>
              <p :if={invite.consumed_by} class="text-xs opacity-70 mt-1">
